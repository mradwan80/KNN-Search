...
__device__
unsigned long long GenerateVertexDistKeySV(int vertex, float dist)
{
...
__global__
void FillDistanceKernelSV(int qnum, int sPixelsNum, int* qscount, int* qsoffset, bool* qkfound, int globalW, int globalH, float* vmMat, float* pvmMat, float* vpos, float* qrads, float cellWidth, int* xfcount, int* xfoffset, int* FragVertex, float* FragDepth, int* qncount, int* qnoffset, int* NbVertex, unsigned long long* NbVertexDist)
{
	...
	if (qspxl < sPixelsNum)
	{
		...
		for (int j = 0; j < xfcount[pxl]; j++)
		{
			...
			if (zdiff * zdiff <= pxlradsqr)
			{
				...
				NbVertexDist[offset + pos] = GenerateVertexDistKeySV(q, qpdst);
